...
def test_main_success(mock_get, mock_patch, monkeypatch):
    ...
    tgt.json.return_value = {
        "policy": {"ID": "t1", "rules": [], "resource": {"ID": "tv"}}
    }
...
def test_main_missing_inputs(monkeypatch):
    ...
    with patch("update_policy.requests.get") as g, patch(
        "update_policy.requests.patch"
    ) as p:
        result = up.main()
...
def test_main_generic_exception(monkeypatch):
    ...
    monkeypatch.setattr(
        up,
        "get_target_policy",
        lambda _id: {"policy": {"ID": "t1", "rules": [], "resource": {"ID": "tv"}}},
    )
    monkeypatch.setattr(
        up,
...
def test_run_as_script(monkeypatch):
    ...
    src.raise_for_status.return_value = None
    src.json.return_value = {
        "policy": {"name": "n", "displayName": "d", "description": "x", "rules": []}
    }
    tgt = MagicMock()
    tgt.raise_for_status.return_value = None
    tgt.json.return_value = {
    ...
    with patch.object(_requests, "get", side_effect=[src, tgt]) as mget, patch.object(
        _requests, "patch", return_value=patch_resp
    ) as mpatch:
