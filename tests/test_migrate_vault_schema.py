...
def test_main_with_config_and_migrate_governance(mock_post, monkeypatch, tmp_path):
    ...
    p = MagicMock(); p.raise_for_status.return_value = None; p.json.return_value = {"ID": "v1"}
    mock_post.return_value = p
...
def test_missing_inputs_and_workspace(monkeypatch):
    ...
    with patch("migrate_vault_schema.requests.get") as g, patch("migrate_vault_schema.requests.post") as p:
        result = mvs.main()
...
def test_http_error_branch(monkeypatch):
    ...
    monkeypatch.setattr(mvs, "WORKSPACE_ID", "ws1", raising=False)
    monkeypatch.setattr(mvs, "get_vault_details", lambda _id: (_ for _ in ()).throw(err))
...
def test_generic_exception_branch(monkeypatch):
    ...
    with patch("builtins.open", mock_open(read_data=schema_json)):
        monkeypatch.setattr(mvs, "create_vault", lambda payload: (_ for _ in ()).throw(Exception("x")))
        with pytest.raises(SystemExit):
            mvs.main()
        assert SystemExit
